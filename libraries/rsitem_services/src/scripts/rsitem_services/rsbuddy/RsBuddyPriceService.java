...
/**
 * This Price Service gets info by downloading the entire RSBuddy Exchange scripts.boe_api Summary and caching the result
 * in the file system and RAM.
 *
 * Will work when the RSBuddy scripts.boe_api is down as long as a cached copy of the data exists in the filesystem and is not considered "outdated"
 */
public class RsBuddyPriceService implements IRsItemPriceService
{
    ...
    public Optional<RsBuddyItem> tryGetItemData(int itemId)
    {
        ...
        if(isCacheOlderThan(outdatedHours) || !dataCache.containsKey(itemId))
            ...
            return Optional.of(dataCache.get(itemId));
    }
    ...
    public Optional<RsBuddyItem> tryGetItemData(String itemName)
    {
        ...
        if(isCacheOlderThan(outdatedHours) || !itemNameIdPairs.containsKey(itemName))
            ...
            return Optional.of(dataCache.get(itemNameIdPairs.get(itemName)));
    }
    ...
    public Optional<Integer> tryGetPrice(String itemName)
    {
        ...
        if(item.isPresent())
            return Optional.of(item.get().getAveragePrice());
        return Optional.empty();
    }
    ...
    private void updateCacheIfNecessary()
    {
        if(isCacheOlderThan(updateIntervalHours))
        {
            ...
            try
            {
                saveUrl(CACHE_FILEPATH, "https://storage.googleapis.com/osbuddy-exchange/summary.json");
                ...
                //General.println("Cache now updated with " + dataCache.size() + " RS Items from the Exchange scripts.boe_api");
            }
            catch (Exception e)
            {
